...
const ChangeUserRole = ({name,email,role, userId,onClose, callFunc}) => {
    ...
    const updateUserRole = async ()=> {
        const fecthUserRole = await fetch(summaryApi.updateUser.url, {
        ...
        const dataResponse = await fecthUserRole.json()
    ...
    return (
        <div className='fixed top-0 bottom-0 left-0 right-0 w-full h-full z-10 flex justify-between items-center bg-slate-200 bg-opacity-30'>
            <div className='mx-auto bg-white shadow-md p-4 w-full max-w-sm'>
                ...
                <button className='w-fit mx-auto block py-1 px-3 rounded-full bg-black text-white' onClick={updateUserRole}>Change Role</button>
            </div>
        </div>
    )
}
